...
class JmhPlugin : Plugin<Project> {
    ...
    override fun apply(project: Project) {
        ...
        createTask(project, JMH_NAME, JmhTask::class.java, Action {
            it.group = JMH_GROUP
    ...
    private fun registerBuildListener(project: Project, extension: JmhPluginExtension) {
        project.gradle.addBuildListener(object : BuildAdapter() {
            override fun projectsEvaluated(gradle: Gradle) {
                ...
                val task = project.tasks.findByName(JMH_JAR_TASK_NAME)
                task.zip64 = extension.zip64
            }
        })
    }

    private fun createJmhRunBytecodeGeneratorTask(project: Project, jmhGeneratedSourcesDir: File,
                                                  extension: JmhPluginExtension, jmhGeneratedResourcesDir: File) =
            createTask(project, "jmhRunBytecodeGenerator", JmhBytecodeGeneratorTask::class.java, Action {
                it.group = JMH_GROUP
    ...
    private fun createJmhCompileGeneratedClassesTask(project: Project, jmhGeneratedSourcesDir: File,
                                                     jmhGeneratedClassesDir: File, extension: JmhPluginExtension) =
            createTask(project, JMH_TASK_COMPILE_GENERATED_CLASSES_NAME, JavaCompile::class.java, Action {
                it.group = JMH_GROUP
    ...
    private fun createShadowJmhJar(project: Project, extension: JmhPluginExtension, jmhGeneratedResourcesDir: File,
        ...
        createTask(project, JMH_JAR_TASK_NAME, clazz, Action {
            it.group = JMH_GROUP
    ...
    private fun createStandardJmhJar(project: Project, extension: JmhPluginExtension, metaInfExcludes: List<String>,
                                     jmhGeneratedResourcesDir: File, jmhGeneratedClassesDir: File,
                                     runtimeConfiguration: Configuration) {
        createTask(project, JMH_JAR_TASK_NAME, Jar::class.java, Action {
            it.group = JMH_GROUP
            it.dependsOn(JMH_TASK_COMPILE_GENERATED_CLASSES_NAME)
